"""
Build MOUAD_LOUHICHI_VIVA_40min  (version VII) on the
...
Usage:  python3 build_v7.py <template.pptx> <out.pptx>
"""
import json
import os
import sys
...
NOTES = json.load(open(os.path.join(HERE, "notes_v7.json")))
...
def limitations_slide(ctx, eyebrow, tabs, active, lims, next_text, notes, title="Limitations"):
    ...
    for i, ((h1, body), (x, y, w, h)) in enumerate(zip(lims, cells)):
        f = [GREEN, "1E5F58", "1E5F58", GREEN][i % 4]
...
def build(template, out):
    ...
    for (fn, name, desc), (x, y, w, h) in zip(brands, cells):
        ...
        if os.path.exists(path):
            ...
            chip(s, x + emu(0.5), y + emu(0.45), lw_, lh_, name, fill=TINT, color=INK, size=30)
    ...
    def diag_hybrid(s, x, y, w, h):
        ...
        chip(s, x + cw + emu(0.3), by, cw, emu(1.0), "Collaborative signals", fill="1E5F58", size=17, sub="ratings · clicks", sub_size=13)
        fy = by + emu(1.45)
    ...
    def diag_graph(s, x, y, w, h):
        ...
        cx2 = x2 + cw / 2
        ellipse(s, cx2 - emu(1.55), gy + emu(0.85), emu(3.1), emu(1.9), fill="E6EEEA", line=GREEN)
    ...
    s, top = content_slide(ctx, "Datasets Used Throughout the Thesis", eyebrow="Experimental Protocol",
                           tabs=PTABS, active="Datasets", notes=N(16, 17), refs=cite(19, 20))
    ...
    by = top + th + emu(0.4)
    bar = rrect(s, L, by, W, min(emu(1.0), CB - by - emu(0.3)), fill=GREEN, radius=emu(0.25))
    ...
    # --- DD3: the same game three times ----------------------------------------
    s, top = content_slide(ctx, "The Same Game, Three Times in This Thesis", eyebrow=DD, tabs=DDTABS, active="In this thesis", notes=N("dd_thesis"), refs=cite(1, 2, 3))
    rows = [["", "Contribution I", "Contribution II", "Contribution III  ·  DyHuCoG"],
            ["Players", "the 11 features of a wine sample", "the 11 features, at each level of the hierarchy", "users, items and contexts (hyperedge members)"],
            ["Value function v(S)", "Silhouette of K-Means using only the features in S", "same, computed per level (coarse regimes, then sub-clusters)", "coalition utility U(S) = α·accuracy + β·diversity + γ·novelty"],
            ["What φ_{j} means", "how much feature j adds to clustering quality", "how importance changes between a partition and its sub-partition", "how much a user, item or context adds to a good recommendation"],
            ["How it is computed", "exact TreeSHAP on a LightGBM surrogate", "exact TreeSHAP, one surrogate per level", "Monte Carlo sampling, M = 50 permutations, refreshed every 10 batches"],
            ["Where it is used", "after training: explanation", "after training: explanation across levels", "**during training: Shapley-weighted hyperedges**"]]
    gf, th = table(s, L, top, W, rows, col_widths=[2.6, 4.7, 4.9, 5.4], size=16, head_size=17, row_h=emu(0.74))
    by = top + th + emu(0.45)
    statement(s, L, by + emu(0.2), W, CB - by - emu(0.5), "One definition, three value functions",
              "The axioms never change. What changes from one contribution to the next is **who the players are** and **what the coalition value measures**. That is why the same explanation can be read the same way across all three chapters.",
              fill=GREEN, size=20)

    # --- C1 game formulation -------------------------------------------------
    s, top = content_slide(ctx, "Clustering as a Cooperative Game", eyebrow=C1, tabs=C1TABS, active="Methodology", notes=N(24), refs=cite(1, 17))
    lw = emu(8.4)
    ...
    bh = emu(2.6)
    box = rrect(s, dx, top, dw, bh, fill=GREEN, radius=emu(0.3))
    textbox(s, dx, top + emu(0.18), dw, emu(0.3), [Para([Run("SHAPLEY VALUE OF FEATURE j  ·  v(S) = SILHOUETTE", font="xbold", size=13, color=YELLOW, spc=1.6)], align="ctr", lnspc=17)])
    equation(s, dx + emu(0.2), top + emu(0.5), dw - emu(0.4), emu(1.55),
             r"\varphi_j = \sum_{S \subseteq F \setminus \{j\}} \dfrac{|S|!\,(|F|-|S|-1)!}{|F|!}\,\left[\, v(S \cup \{j\}) - v(S) \,\right]", size=21, color=WHITE)
    textbox(s, dx, top + bh - emu(0.5), dw, emu(0.35), [Para([Run("same formula as the deep dive: players are now the features", font="body", size=15, color="DCE7E3")], align="ctr", lnspc=18)])
    textbox(s, dx, top + bh + emu(0.3), dw, emu(0.35), [Para([Run("FROM THE BAND TO THE CLUSTERING GAME", font="xbold", size=13, color=ORANGE, spc=1.5)], lnspc=16)])
    rows = [["", "Band example", "Clustering game (C1)"],
            ["Players", "Guitar, Voice, Drums", "the 11 chemical features"],
            ["Coalition S", "who plays the gig", "which features K-Means may use"],
            ["v(S)", "money earned", "Silhouette of K-Means on X_{S}"],
            ["φ_{j}", "fair share of the pay", "feature j's share of clustering quality"]]
    gf, th = table(s, dx, top + bh + emu(0.65), dw, rows, col_widths=[1.5, 2.4, 3.2], size=15, head_size=15, row_h=emu(0.42), pad=0.07)
    ny = top + bh + emu(0.65) + th + emu(0.3)
    bar = rrect(s, dx, ny, dw, CB - ny - emu(0.3), fill=None, line=ORANGE, line_w=2.5, radius=emu(0.25))
    shape_text(bar, [Para(md_runs("**2^{|F|} coalitions:** for 11 wine features that is 2,048 K-Means runs per evaluation; for large data, not feasible.", size=16, color=INK), align="ctr", lnspc=21)],
               anchor="ctr", insets=(emu(0.3), emu(0.05), emu(0.3), emu(0.05)))
    ...
    fx = L + lw + emu(0.5)
    figure_frame(s, TF("fig5_1_wine_global.png"), fx, top, R - fx, CB - top - emu(0.3),
                 caption="Fig. 5.1 (thesis): global SHAP feature importance for the wine clustering surrogate")

    # --- C1 results: cluster signatures (thesis Fig. 5.2) ---------------------
    s, top = content_slide(ctx, "Cluster-Specific Explanatory Signatures", eyebrow=C1, tabs=C1TABS, active="Results", notes=N(29), refs=cite(1))
    fh = emu(2.45)
    figure_frame(s, TF("fig5_2_wine_clusters.png"), L, top, W, fh, caption="Fig. 5.2 (thesis): SHAP profiles of the three wine clusters, same feature space, different weights", pad=0.12)
    ...
    ty = top + fh + emu(0.3)
    th = emu(1.35)
    cells = grid(3, 3, L, ty, W, th, gap=emu(0.35))
    for (h1, body, f), (x, y, w, h) in zip(sig, cells):
        box = rrect(s, x, y, w, h, fill=f, radius=emu(0.28))
        fit_textbox(s, x + emu(0.4), y + emu(0.22), w - emu(0.8), h - emu(0.35),
                    [Para([Run(h1.upper(), font="xbold", size=13, color=YELLOW if f != ORANGE else WHITE, spc=1.6)], lnspc=17),
                     Para([Run(body, font="xbold", size=19, color=WHITE)], lnspc=24, spcbef=6)], anchor="t", min_scale=0.7)
    cy = ty + th + emu(0.5)
    light_card(s, L, cy + emu(0.25), W, CB - cy - emu(0.4), label="What the signatures show",
               paras=bullets(["The three clusters show **distinct explanatory signatures**: the explanation is consistent, not a single summary number.",
                              "The **same small set of variables recurs** across clusters, with **different relative weights** within each; local force plots break any single wine down into per-feature contributions."], size=18, gap0=4), pad=(0.55, 0.6, 0.5, 0.2))
    ...
    rq_slide(ctx, C2, C1TABS, "Objectives", "RQ2",
             ...
             [("O1  ·  A truly multi-level workflow", "Recursive clustering with **level-specific surrogates**, not a rerun of the single-level pipeline.", "Workflow, Fig. 6.3"),
              ...
              ("O3  ·  Validation at scale on new data", "Beijing Multi-Site Air Quality: **383,585 hourly records**, a domain far from wine.", "Fig. 6.1, 6.2, regimes")],
             N(37))
    ...
    # --- C2 results: regimes -------------------------------------------------
    s, top = content_slide(ctx, "Three Atmospheric Regimes", eyebrow=C2, tabs=C1TABS, active="Results", notes=N(42), refs=cite(2))
    ...
    th = emu(2.75)
    cells = grid(3, 3, L, top, W, th, gap=emu(0.35))
    for (tag, h1, body, f), (x, y, w, h) in zip(regs, cells):
        box = rrect(s, x, y, w, h, fill=f, radius=emu(0.3))
        fit_textbox(s, x + emu(0.4), y + emu(0.3), w - emu(0.8), h - emu(0.5),
                    [Para([Run(tag.upper(), font="xbold", size=13, color=YELLOW if f != ORANGE else WHITE, spc=1.6)], lnspc=17),
                     Para([Run(h1, font="title", size=26, color=WHITE, spc=-1.2)], lnspc=30, spcbef=6),
                     Para([Run(body, font="body", size=17, color="F7F2EA")], lnspc=22, spcbef=8)], anchor="t", min_scale=0.7)
    fy = top + th + emu(0.3)
    fh = emu(2.9)
    figure_frame(s, TF("fig6_2_beijing_force.png"), L, fy, W, fh, caption="Fig. 6.2 (thesis): local SHAP force plots for representative observations of each regime", pad=0.15)
    gy = fy + fh + emu(0.25)
    bar = rrect(s, L, gy, W, CB - gy - emu(0.3), fill=TINT, radius=emu(0.25))
    shape_text(bar, [Para(md_runs("The framework shows not only **that these regimes exist**, but **which variable combinations define them**.", size=20, color=INK), align="ctr", lnspc=25)],
               anchor="ctr", insets=(emu(0.5), emu(0.05), emu(0.5), emu(0.05)))
    ...
    s, top = content_slide(ctx, "Generalisation and Comparison", eyebrow=C2, tabs=C1TABS, active="Results", notes=N(44), refs=cite(7, 18))
    ...
    rows = [["Objective", "What it delivers", "Where it is shown"],
            ...
            ["O3  ·  Accuracy, coverage and diversity together", "Better NDCG and Recall **and** higher Coverage and ILD than the strongest baseline, on two datasets.", "Tables 7.1 to 7.6"]]
    gf, th = table(s, L, ty, W, rows, col_widths=[4.6, 7.4, 3.6], size=18, head_size=17, row_h=emu(0.9))
    ...
    # --- C3 results: ranking figure (thesis Fig. 7.2) ---------------------------
    s, top = content_slide(ctx, "Ranking Quality Across All Baselines", eyebrow=C3, tabs=C1TABS, active="Results", notes=N("c3_fig72"), refs=cite(3))
    fh = emu(5.0)
    figure_frame(s, TF("fig7_2_ranking.png"), L, top, W, fh, caption="Fig. 7.2 (thesis): NDCG@20 and Recall@20 of all models on MovieLens-1M and Amazon-Book", pad=0.15)
    gy = top + fh + emu(0.3)
    cw = (W - emu(0.4)) / 2
    bh = CB - gy - emu(0.3)
    box = rrect(s, L, gy, cw, bh, fill=GREEN, radius=emu(0.25))
    shape_text(box, [Para([Run("WHAT THE FIGURE SHOWS", font="xbold", size=12, color=YELLOW, spc=1.5)], lnspc=15),
                     Para(md_runs("A **monotone ladder**: classical < neural < graph < hypergraph < DyHuCoG, on both datasets; the gap to HPCF is largest on Amazon-Book.", size=17, color=WHITE), lnspc=22, spcbef=6)],
               anchor="ctr", insets=(emu(0.4), emu(0.15), emu(0.4), emu(0.15)))
    box = rrect(s, L + cw + emu(0.4), gy, cw, bh, fill=TINT, radius=emu(0.25))
    shape_text(box, [Para([Run("WHY THE ORDER MAKES SENSE", font="xbold", size=12, color=ORANGE, spc=1.5)], lnspc=15),
                     Para(md_runs("Each family adds structure: pairwise graph, then hyperedges, then **Shapley-weighted hyperedges with context**. The improvement is **cumulative and explainable**, not a tuning artefact.", size=17, color=INK), lnspc=22, spcbef=6)],
               anchor="ctr", insets=(emu(0.4), emu(0.15), emu(0.4), emu(0.15)))

    # --- C3 results: coverage & diversity (thesis Fig. 7.3) ---------------------
    s, top = content_slide(ctx, "Coverage & Diversity: No Accuracy Trade-Off", eyebrow=C3, tabs=C1TABS, active="Results", notes=N(59), refs=cite(3, 23))
    lw = emu(9.4)
    figure_frame(s, TF("fig7_3_coverage_ild.png"), L, top, lw, CB - top - emu(1.6), caption="Fig. 7.3 (thesis): catalogue coverage and intra-list diversity, HPCF vs DyHuCoG", pad=0.15)
    dx = L + lw + emu(0.5)
    dw = R - dx
    gains = [("Coverage · MovieLens-1M", "0.342 → 0.397", "+16.1 %"), ("ILD · MovieLens-1M", "0.461 → 0.516", "+11.9 %"),
             ("Coverage · Amazon-Book", "0.259 → 0.336", "+29.7 %"), ("ILD · Amazon-Book", "0.535 → 0.602", "+12.5 %")]
    cells = grid(4, 2, dx, top, dw, CB - top - emu(1.6), gap=emu(0.25), vgap=emu(0.25))
    for i, ((lab, v, g), (x, y, w, h)) in enumerate(zip(gains, cells)):
        f = GREEN if i % 3 == 0 else "1E5F58"
        box = rrect(s, x, y, w, h, fill=f, radius=emu(0.25))
        shape_text(box, [Para([Run(lab.upper(), font="xbold", size=11, color=YELLOW, spc=1.2)], align="ctr", lnspc=14),
                         Para([Run(g, font="xbold", size=28, color=WHITE)], align="ctr", lnspc=32, spcbef=6),
                         Para([Run(v, font="body", size=14, color="DCE7E3")], align="ctr", lnspc=17, spcbef=2)],
                   anchor="ctr", insets=(emu(0.1), emu(0.1), emu(0.1), emu(0.1)))
    gy = CB - emu(1.3)
    bar = rrect(s, L, gy, W, emu(1.0), fill=GREEN, radius=emu(0.25))
    shape_text(bar, [Para(md_runs("**Smaller filter-bubble effect and more discovery for the user**, while NDCG and Recall also improve: **accuracy is not traded for diversity**.", size=20, color=WHITE), align="ctr", lnspc=25)],
               anchor="ctr", insets=(emu(0.5), emu(0.05), emu(0.5), emu(0.05)))
    ...
    s, top = content_slide(ctx, "Cold-Start, Cross-Dataset Robustness and Interpretability", eyebrow=C3, tabs=C1TABS, active="Results", notes=N(63), refs=cite(3))
    cw = (W - emu(0.4)) / 2
    ch = emu(3.2)
    ...
    fw = emu(7.4)
    figure_frame(s, TF("fig7_4_waterfall.png"), L, gy, fw, CB - gy - emu(0.3), caption="Fig. 7.4 (thesis): SHAP waterfall for one recommendation", pad=0.12)
    light_card(s, L + fw + emu(0.4), gy + emu(0.25), W - fw - emu(0.4), CB - gy - emu(0.55), label="Interpretability & popularity bias",
               paras=bullets(["A **SHAP waterfall** breaks a recommendation down into ranking, diversity, context and preference contributions.",
                              "Shapley measures **marginal utility, not raw frequency**: weak but informative interactions keep their influence, which counters popularity bias."], size=18, gap0=6), pad=(0.5, 0.65, 0.45, 0.25))
    ...
    # ======================================================================
    ctx.section = None
    section_slide(ctx, "B", "Backup Slides", "Full tables from Chapter 7, for questions",
                  [("Table 7.1", "All baselines with mean ± standard deviation over five seeds."),
                   ("Table 7.6", "Paired t-tests against every baseline, Holm–Bonferroni thresholds, effect sizes."),
                   ("Tables 7.3 & 7.4", "Training time, inference latency, memory; Monte Carlo convergence.")],
                  notes=N("backup_intro"))
    ...
    lw = emu(11.2)
    rows = [["Model", "Train (s) ML-1M", "Infer (ms)", "Mem (GB)", "Train (s) Amazon", "Infer (ms)", "Mem (GB)", "× MF"],
            ["MF", "485", "0.54", "2.1", "2,202", "2.46", "8.5", "1.00"],
            ["NCF", "675", "0.76", "2.7", "3,073", "3.42", "11.1", "1.40"],
            ["LightGCN", "786", "0.88", "3.2", "3,645", "4.05", "12.8", "1.64"],
            ["RecDCL", "968", "1.02", "3.6", "4,527", "5.04", "15.6", "1.97"],
            ["HCCF", "952", "1.06", "3.8", "4,419", "4.92", "15.3", "1.98"],
            ["HPCF", "1,125", "1.18", "4.1", "5,234", "5.48", "16.8", "2.21"],
            ["DyHuCoG", "2,000", "1.84", "4.4", "9,279", "8.52", "17.9", "3.44"]]
    al = ["l"] + ["ctr"] * 7
