"""
Build MOUAD_LOUHICHI_VIVA_40min  (version 13) on the
...
Version 13 keeps the v11 speech and slide content, and changes only the
look: the blue now matches the reference viva deck (its slides reference
theme accent1 = 4472C4 2,160 times), the orange accent becomes a deep
teal, and the two decorative illustrations on the title slide are gone.
The header line dropped in v12 stays dropped.

Usage:  python3 build_v13.py <template.pptx> <out.pptx>
"""
import json
import os
import sys
...
palette_blue.apply(tpl, layouts, swap=palette_blue.SWAP_V13)
...
def build(template, out):
    ...
    def diag_hybrid(s, x, y, w, h):
        ...
        chip(s, x + cw + emu(0.3), by, cw, emu(1.0), "Collaborative signals", fill="2F5597", size=17, sub="ratings · clicks", sub_size=13)
        fy = by + emu(1.45)
    ...
    def diag_graph(s, x, y, w, h):
        ...
        cx2 = x2 + cw / 2
        ellipse(s, cx2 - emu(1.55), gy + emu(0.85), emu(3.1), emu(1.9), fill="D9E2F3", line=GREEN)
