"""
Build MOUAD_LOUHICHI_VIVA_40min  (version 16) on the
...
Version 14 keeps the v11 speech and slide content, and changes only the
look. On top of v13: the slide background is white instead of the
template's warm cream, and the decorative accent drawn immediately right
of every headline (the asterisks, sparkles, flowers) is gone. The beige
card fills and hairlines are untouched.

Usage:  python3 build_v16.py <template.pptx> <out.pptx>
"""
import json
import os
import sys
...
NOTES = json.load(open(os.path.join(HERE, "notes_v16.json")))
...
def limitations_slide(ctx, eyebrow, tabs, active, lims, next_text, notes, title="Limitations"):
    ...
    for i, ((h1, body), (x, y, w, h)) in enumerate(zip(lims, cells)):
        f = [GREEN, "2F5597", "2F5597", GREEN][i % 4]
...
def build(template, out):
    ...
    for i, (num, head, sub) in enumerate(parts):
        ...
        shape_text(box, [Para([Run(num, font="xbold", size=30, color=YELLOW if dark else ORANGE)], lnspc=34),
                         ...
                         Para([Run(sub, font="body", size=17, color="D6E4F7" if dark else MUTED)], lnspc=21, spcbef=6)],
    ...
    for (head, q, sub), (x, y, w, h) in zip(qs, cells):
        card(s, x, y, w, h, label=head, paras=[P(q, size=23, color=WHITE, font="bold"),
                                                 Para([Run(sub, font="body", size=18, color="BDD0EA")], lnspc=23, spcbef=14)],
    ...
    def diag_content(s, x, y, w, h):
        ph = w * 5.94 / 8.75
        picture(s, os.path.join(FIGS, "ctx_content.png"), x, y + (h - ph) / 2, w=w, h=ph)
    ...
    def diag_cf(s, x, y, w, h):
        ph = w * 5.94 / 8.75
        picture(s, os.path.join(FIGS, "ctx_cf.png"), x, y + (h - ph) / 2, w=w, h=ph)
    ...
    def diag_hybrid(s, x, y, w, h):
        ph = w * 5.94 / 8.75
        picture(s, os.path.join(FIGS, "ctx_mf.png"), x, y + (h - ph) / 2, w=w, h=ph)
    ...
    def diag_graph(s, x, y, w, h):
        ph = w * 5.94 / 8.75
        picture(s, os.path.join(FIGS, "ctx_hyper.png"), x, y + (h - ph) / 2, w=w, h=ph)
    ...
    textbox(s, L, fy + emu(1.68), lw, emu(0.34), [Para([Run("average marginal contribution over every arrival order", font="body", size=15, color="D6E4F7")], align="ctr", lnspc=18)])
    ...
    regs = [("Regime A", "temp · dew point · ozone", GREEN), ("Regime B", "CO · SO₂ · PM · wind", "2F5597"), ("Regime C", "clean air · weather", ORANGE)]
    ...
    equation(s, dx + emu(0.2), fy + emu(1.5), dw - emu(0.4), emu(0.5),
             r"w_{c'} = |c'| \,/\, |c|, \qquad \varepsilon_j \to 0 \ \text{ under perfect surrogate fidelity}", size=14, color="D6E4F7")
    ...
    kpi(s, *cells[1], "≈ 0.63", "**Silhouette** · much stronger separation than wine", fill="2F5597")
    ...
    groups = [("Users", "U", GREEN), ("Items", "I", "2F5597"), ("Contexts", "C", ORANGE)]
    ...
    kpi(s, *cells[1], "1.84 ms", "inference / query on ML-1M **(8.52 ms Amazon)**", fill="2F5597", vsize=36)
    ...
    stats = [(r"t = 46.38", "paired t-test vs HPCF · **df = 6,039**", GREEN, YELLOW),
             (r"d_z = 1.33", "**Cohen's d_{z}** · a large effect size", "2F5597", YELLOW),
    ...
    for ri, f in ((1, GREEN), (2, "2F5597"), (3, ORANGE)):
    ...
    gy = top + th + emu(0.4)
    steps = [("Explain", "post-hoc, single level", GREEN), ("Scale", "post-hoc, hierarchical", "2F5597"), ("Guide", "in-training signal", ORANGE)]
    n = 3
    bw = emu(4.4)
    gap = (W - n * bw) / (n - 1)
    bh = emu(0.9)
    ...
    lims = [("Computational", "Exact Shapley is not feasible; every contribution relies on approximation, surrogates or limited reporting.", GREEN),
            ("Methodological", "Clustering depends on surrogate fidelity; recommendation depends on stable approximate contributions and useful context.", "2F5597"),
