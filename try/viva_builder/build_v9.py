...
NOTES = json.load(open(os.path.join(HERE, "notes_v9.json")))
...
def build(template, out):
    ...
    DS_CAPTION = "Table 4.1 (thesis): core dataset statistics; density is defined only for the recommendation datasets."
    ...
    source_caption(s, L, top + th + emu(0.08), W, DS_CAPTION)
    by = top + th + emu(0.55)
    ...
    source_caption(s, L, top + th + emu(0.08), W, "Table 5.1 (thesis): theoretical comparison of SHAP and LIME.")
    by = top + th + emu(0.55)
    ...
    source_caption(s, L, top + th + emu(0.06), W, "Table 6.2 (thesis): cross-dataset comparison of the Chapter 5 and Chapter 6 frameworks  ·  literature comparison: Table 6.3 (thesis).")
    by = top + th + emu(0.5)
    cw = (W - emu(0.4)) / 2
    ...
    source_caption(s, L, top + emu(0.25) + ch + emu(0.03), W, "Table 7.5 (thesis): cold-start and cross-dataset robustness (Yelp2018 is an auxiliary robustness benchmark).")
    gy = top + emu(0.25) + ch + emu(0.45)
    fw = emu(7.4)
