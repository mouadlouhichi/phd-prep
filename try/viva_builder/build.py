...
NOTES = json.load(open(os.path.join(HERE, "notes_original.json")))


def N(*orig):
    """Speaker notes: merge notes of the original slide numbers."""
    return "\n\n".join(NOTES[str(o)] for o in orig if str(o) in NOTES)
...
def build(template, out):
    ...
    textbox(s, L, emu(3.55), W, emu(0.5), [Para([Run("Five parts  ·  each contribution follows the same rhythm: objectives → methodology → results → findings",
                                                     font="body", size=21, color=MUTED)], align="ctr", lnspc=26)])
    parts = [
        ("01", "Introduction", "Motivation · Actionable insight · Why XAI matters"),
        ("02", "Context & Problematic", "Paradigms · Limitations · Research questions"),
        ...
        ("06", "Contribution III", "DyHuCoG — cooperative game on hypergraphs"),
    ...
    # ======================================================================
    section_slide(ctx, "01", "Introduction", "Why explainability is a first-class requirement for recommender systems",
                  [("Motivation", "Opaque AI systems mediate what billions of people see, buy and watch every day — accuracy without transparency."),
                   ("Actionable insight", "An explanation is useful when it names a modifiable, domain-level factor a designer can act upon."),
                   ("Research context", "From matrix factorisation to hypergraph recommenders: each step raised expressiveness and lowered transparency.")],
                  notes=N(3))
    ...
    qs = [("Ubiquity", "How do opaque AI systems shape what billions of users see, buy and watch every day?",
           "Recommenders shape news, study, health and credit decisions — market > $15B by 2029."),
          ("The Black Box", "Why do state-of-the-art recommenders and clustering pipelines remain opaque to users and designers?",
           "Deep & graph models: opaque, non-auditable. EU AI Act: high-risk → must explain."),
          ("Toward Trust", "How can transparency be built as part of the model, instead of being bolted on afterwards?",
           "Accountable, auditable, modifiable insight — actionable, not just accurate.")]
    ...
    shape_text(bar, [Para(md_runs("**The core tension:** as models gain expressive power, they lose the transparency needed for trustworthy deployment — "
                                  "this thesis treats **accuracy and interpretability as objectives to be reconciled**, not traded against one another.",
    ...
    fit_textbox(s, L, top, lw, CB - top - emu(0.2), [
        ...
        *bullets([
            "An explanation is actionable when it identifies **at least one modifiable factor** whose change is associated with a **specifiable change in model output**…",
            "… and that factor is **expressible in the semantic vocabulary of the task domain**.",
            "Accessibility means the domain's own terms: a physicochemical variable (wine), a pollution indicator (air quality), a preference signal (recommendation) — **not an opaque latent code**.",
            "**Why it matters:** an explanation that identifies a modifiable driver supports **intervention**, not merely description.",
        ], size=21)
    ], min_scale=0.7)
    ...
    steps = [("Modifiable factor", GREEN), ("Change in model output", GREEN), ("Is it actionable?", INK), ("Prescriptive lever", ORANGE)]
    n = len(steps)
    bw = emu(1.75)
    gap = (dw - n * bw) / (n - 1)
    by = top + emu(1.4)
    ...
    textbox(s, L, ay + emu(0.1), W, emu(0.4), [Para(md_runs("**Interpretability deficit grows:** each step improved ranking but replaced an opaque latent code by an opaque message-passing mechanism — non-modifiable factors.", size=18, color=ORANGE), align="ctr", lnspc=23)])
    # bottom: two cards
    cy = ay + emu(0.85)
    chh = CB - cy - emu(0.1)
    cw = (W - emu(0.4)) / 2
    card(s, L, cy + emu(0.25), cw, chh - emu(0.25), label="Why the deficit matters",
         paras=bullets(["**Undermines user trust** — outputs observed without understanding why.",
                        "**Constrains debugging** and scientific learning.",
                        "**Collides with regulation** — EU AI Act, OECD principles, GDPR.",
    ...
    light_card(s, L + cw + emu(0.4), cy + emu(0.25), cw, chh - emu(0.25), label="What this thesis argues",
               paras=bullets(["Recommenders evolved from similarity filters to **representation learning on sparse, high-dimensional, dynamic data**.",
                              "Hypergraph models add higher-order user–item–context relations but assume **uniform message importance**.",
                              "A principled attribution mechanism is needed — and it should be **part of the modelling logic**, not an afterthought."],
    ...
    shape_text(bar, [Para(md_runs("AI recommendation mediates what **billions** see, buy and listen to every day. Each system is remarkably **accurate yet opaque** — "
                                  "its reasoning is hidden from the user. That ubiquity-plus-opacity is **precisely the gap this thesis targets**.", size=22, color=INK),
                          align="ctr", lnspc=29)], anchor="ctr", insets=(emu(0.6), emu(0.1), emu(0.6), emu(0.1)))

    # ======================================================================
    # SECTION 2 — CONTEXT & PROBLEMATIC
    # ======================================================================
    section_slide(ctx, "02", "Context & Problematic", "Paradigm landscape, structuring limitations and the five research questions",
                  [("Paradigms", "Collaborative, content-based, hybrid, matrix factorisation and graph/hypergraph recommenders — each stronger, each less transparent."),
                   ("Problematic", "Three structuring limitations: lack of explainability, difficulty of scaling, weak integration of explanation into learning."),
                   ("Contributions", "Five research questions answered by three cumulative contributions under one cooperative-game perspective.")],
                  notes=N(8))

    # --- Paradigms -----------------------------------------------------------
    s, top = content_slide(ctx, "Recommendation & Clustering Paradigms", eyebrow="Context & Problematic",
                           tabs=["Paradigms", "Problematic", "Contributions"], active="Paradigms", notes=N(9))
    lw = emu(8.7)
    fit_textbox(s, L, top, lw, CB - top - emu(0.2), bullets([
        "**Collaborative filtering** – users who behaved similarly will value similar items (user-/item-based).",
        "**Content-based** – recommends items sharing attributes with a user profile.",
        "**Hybrid** – combines collaborative and content signals.",
        "**Matrix factorisation** – R ≈ PQ^{T}: compact but opaque latent factors.",
        "**Graph-based** – interaction graph with neighbourhood propagation (LightGCN, hypergraph).",
        "Each step strengthens the modelling but complicates interpretation: MF made latent dimensions opaque; graph models kept importance implicit; hypergraphs assumed **uniform message importance**.",
    ...
    for i, (t, f) in enumerate(steps):
        ...
        if i < n - 1:
            arrow(s, x + bw + emu(0.05), by + bh / 2, x + bw + gap - emu(0.05), by + bh / 2, color=GREEN, w=2.25)
    ey = by + bh + emu(0.4)
    textbox(s, dx, ey, dw, emu(0.5), [Para(md_runs("**Example:** liked sci-fi + Nolan  →  surface films with similar genre / cast / keywords", size=17, color=INK), align="ctr", lnspc=22)])
    cw = (dw - emu(0.3)) / 2
    ...
    shape_text(bar, [Para(md_runs("Explainable because the factors are **domain-level and modifiable** — the property lost when latent factors and message passing take over.", size=17, color=WHITE), align="ctr", lnspc=22)],
    ...
    s, top = content_slide(ctx, "Limitations of Classical Recommenders & Unsupervised Models", eyebrow="Context & Problematic",
                           tabs=["Paradigms", "Problematic", "Contributions"], active="Problematic", notes=N(10))
    lims = [("Data sparsity & scalability", "The user–item matrix is overwhelmingly empty — learning is starved of signal."),
            ("Cold-start", "New users and items are structurally disadvantaged: no history to learn from."),
            ("Popularity bias & lack of diversity", "Exposure begets interaction, begets exposure — a filter-bubble loop."),
            ("Absence of interpretability", "The most fundamental limit — and the one this thesis targets.")]
    ...
    light_card(s, L, cy + emu(0.25), W, CB - cy - emu(0.45), label="For clustering specifically, the situation is even harder",
               paras=bullets(["Methods favour **local OR global** explanation — not both.",
                              "They **struggle to scale**, and explanations rarely stay **coherent across resolutions**.",
                              "**The gap:** no faithful local + global explanation, poor scaling, and incoherence across levels — precisely what this thesis works on."],
                             size=21, gap0=8), pad=(0.55, 0.7, 0.5, 0.3))

    # --- Three structuring limitations ---------------------------------------
    s, top = content_slide(ctx, "Three Structuring Limitations — Problem Statement", eyebrow="Context & Problematic",
                           tabs=["Paradigms", "Problematic", "Contributions"], active="Problematic", notes=N(11))
    probs = [("1", "Lack of explainability", "Complex models remain hard to interpret **faithfully and actionably**."),
             ("2", "Difficulty of scaling", "Local explanations do not transfer naturally to **hierarchical structures or large datasets** — a method that works on a toy partition may collapse on hundreds of thousands of nested records."),
             ("3", "Weak integration into learning", "Most explanations stay **post-hoc**: they do not shape model dynamics or the **accuracy–diversity–context trade-off**.")]
    ...
    fit_textbox(s, L + emu(0.5), gy + emu(0.25), W - emu(1.0), CB - gy - emu(0.15) - emu(0.5),
                [Para([Run("THESIS GAP", font="xbold", size=14, color=ORANGE, spc=1.8)], lnspc=18),
                 P("The literature still lacks **a single cooperative-attribution framework** that explains clustering faithfully, stays coherent under hierarchy, and then operates as an **in-training signal** in recommendation. "
                   "**Claim:** Shapley-value attribution can be that single framework.", size=21, spcbef=6)], anchor="ctr", min_scale=0.7)

    # --- Research questions --------------------------------------------------
    s, top = content_slide(ctx, "Research Questions (RQ1–RQ5) and Overall Aim", eyebrow="Context & Problematic",
                           tabs=["Paradigms", "Problematic", "Contributions"], active="Problematic", notes=N(12))
    aim = rrect(s, L, top, W, emu(0.95), fill=GREEN, radius=emu(0.25))
    shape_text(aim, [Para(md_runs("**Aim** — develop, justify and evaluate a cooperative-game-theoretic perspective for XAI in clustering and recommendation, "
                                  "using Shapley attribution as both an **explanatory mechanism** and an **in-training signal**.", size=20, color=WHITE), align="ctr", lnspc=26)],
    ...
    rqs = [("RQ1", "How can Shapley values explain black-box clustering **faithfully at instance and cluster level**?", "C1"),
           ("RQ2", "How can this extend to **large-scale, hierarchical clustering** without losing tractability or consistency?", "C2"),
           ("RQ3", "Can cooperative attribution move **beyond post-hoc** and enter the learning dynamics of graph recommenders?", "C3"),
           ("RQ4", "Can a recommender **jointly optimise ranking accuracy, context and diversity** when importance is estimated by a cooperative-game utility?", "C3"),
           ("RQ5", "What emerges when clustering explanation and recommendation learning are **two stages of one cooperative-game perspective**?", "Thesis")]
    y = top + emu(1.2)
    ...
    s, top = content_slide(ctx, "The Three Contributions", eyebrow="Context & Problematic",
                           tabs=["Paradigms", "Problematic", "Contributions"], active="Contributions", notes=N(13))
    cons = [("C1", "Explainable black-box clustering", "PCA → K-Means → LightGBM → TreeSHAP pipeline.", "Wine Quality", "Faithful instance- and cluster-level feature attribution.", "RQ1"),
            ("C2", "Enhanced multi-level XAI", "Large-scale clustering with cross-level SHAP aggregation (Prop. 6.1).", "Beijing Air Quality", "Hierarchical attribution consistency under scale.", "RQ2"),
    ...
    shape_text(bar, [Para(md_runs("**Thesis claim:** cooperative game theory functions as a **shared attribution perspective** for explanation, optimisation and intervention — three cumulative steps, not three unrelated papers.", size=21, color=INK), align="ctr", lnspc=27)],
               anchor="ctr", insets=(emu(0.5), emu(0.1), emu(0.5), emu(0.1)))

    # --- Our thesis ----------------------------------------------------------
    s, top = content_slide(ctx, "Our Thesis in One View", eyebrow="Context & Problematic",
                           tabs=["Paradigms", "Problematic", "Contributions"], active="Contributions", notes=N(14))
    lw = emu(7.6)
    fit_textbox(s, L, top, lw, CB - top - emu(0.2), [
        H("Thesis", size=25, color=GREEN),
        P("Cooperative game theory (Shapley) is **one shared, explainable attribution perspective**.", size=21, spcbef=6),
        ...
        *bullets(["**C1** – explains black-box clustering faithfully (wine).",
                  "**C2** – keeps explanation coherent under scale and hierarchy (Beijing).",
    ...
    dx = L + lw + emu(0.7)
    dw = R - dx
    n1 = chip(s, dx + dw * 0.15, top + emu(0.05), dw * 0.7, emu(1.0), "Everyday AI recommenders are powerful but opaque", fill=GREEN, size=18, radius=emu(0.2))
    n2 = chip(s, dx + dw * 0.08, top + emu(1.65), dw * 0.84, emu(1.2), "Our thesis: cooperative game theory (Shapley) as a shared, explainable attribution perspective", fill=INK, size=18, radius=emu(0.2))
    ...
    shape_text(bar, [Para(md_runs("Same axioms · same vocabulary · same value function logic — **explanation as method, not commentary.**", size=17, color=INK), align="ctr", lnspc=22)],
    ...
    section_slide(ctx, "03", "Experimental Protocol", "One shared protocol so that the three contributions can be read together",
                  [("Datasets", "Wine Quality and Beijing Air Quality for clustering; MovieLens-1M and Amazon-Book for recommendation."),
                   ...
                   ("Reproducibility", "Fixed seeds, temporal user-level splits, early stopping, ordinary academic hardware (i9-14900K + RTX 4090).")],
                  notes=N(15))
    ...
    rows = [["Dataset", "Scale", "Type", "Role", "Why it was chosen"],
            ["Wine Quality (vinho verde)", "4,898 × 11", "Tabular, numeric", "C1 – single-level clustering", "11 interpretable physicochemical features; small, dense, chemically correlated → attribution in a trusted semantic space."],
            ["Beijing Multi-Site Air Quality", "383,585 × 11", "Tabular, pollutant + meteorology", "C2 – multi-level clustering", "Hourly records → tests scale, noise and temporal / meteorological structure."],
            ["MovieLens-1M", "6,040 u / 3,706 i / 1.0 M int.", "Implicit feedback (density 0.0447)", "C3 – DyHuCoG", "Benchmark-standard recommendation with established baselines."],
            ["Amazon-Book", "52,643 u / 91,599 i / 3.0 M int.", "Implicit feedback (density 0.0006)", "C3 – DyHuCoG", "Deliberately sparse → stress-tests Shapley weighting when the signal is weak."]]
    ...
    shape_text(bar, [Para(md_runs("The **0.0447 vs 0.0006** density contrast is intentional: the claim is that **gains are largest where data are weakest**.", size=21, color=WHITE), align="ctr", lnspc=27)],
               anchor="ctr", insets=(emu(0.5), emu(0.1), emu(0.5), emu(0.1)))
    ...
    card(s, L, top + emu(0.25), cw, ch, label="Clustering (C1, C2)",
         paras=bullets(["**Five-fold cross-validation** for surrogate / attribution stability.",
                        "Feature standardisation; PCA used only as a geometric and visual diagnostic.",
    ...
    light_card(s, L + cw + emu(0.4), top + emu(0.25), cw, ch, label="Recommendation (C3)",
               paras=bullets(["**User-level temporal split** — 70 % train / 10 % validation / 20 % test.",
                              ...
                              "Popularity-aware negative sampling **q(i) ∝ f_{i}^{η}** for harder contrasts.",
    ...
    light_card(s, L, cy + emu(0.25), W, CB - cy - emu(0.45), label="Notes",
               paras=bullets(["Altair for interactive SHAP visualisation; metrics reported at K ∈ {5, 10, 20}.",
                              ...
                              "**Everything stays within ordinary academic compute — nothing requires industrial infrastructure.**"], size=20, gap0=8), pad=(0.55, 0.7, 0.5, 0.3))
    ...
    card(s, L, top + emu(0.25), cw, ch, label="Gap",
         paras=bullets(["Shapley explanation is **dominant in supervised tasks**, but clustering remains comparatively under-explained.",
                        "Existing clustering-interpretability methods privilege **local or global** explanation, not both.",
                        "They often fail to scale or to preserve **coherence across clusters**.",
                        "Clustering is the hardest test bed: the model creates its own structure, so cluster meaning must be inferred after the fact."],
    ...
    numbered_rows(s, x2 + emu(0.5), top + emu(2.05), cw - emu(1.0), [
        ...
        ("O2", "Preserve the semantics of the **original feature space** — density, pH, acidity, sulfur dioxide, alcohol — not a reduced latent space."),
    ...
    card(s, L, cy + emu(0.25), cw, ch, label="How",
         paras=bullets(["Once K-Means produces cluster labels, train a **LightGBM multiclass surrogate** to predict them from the original features.",
                        "Apply **TreeSHAP** to the surrogate — fast, exact tree-based attribution in the original semantic feature space.",
    ...
    light_card(s, L + cw + emu(0.4), cy + emu(0.25), cw, ch, label="Why",
               paras=bullets(["Direct TreeSHAP on K-Means is impossible — it explains **tree models, not centroids**.",
                              ...
                              "The surrogate preserves the **chemistry / pollution vocabulary** that makes the analysis actionable.",
                              "**Validity condition:** surrogate fidelity is high — **macro-F1 ≈ 0.82** as the floor."], size=20, gap0=10), pad=(0.55, 0.7, 0.5, 0.3))

    # --- C1 pipeline ---------------------------------------------------------
    s, top = content_slide(ctx, "Pipeline in Five Stages", eyebrow=C1, tabs=C1TABS, active="Methodology", notes=N(26))
    stages = [("1", "PCA", "Stabilise geometry + visual diagnostic — **NOT** the explanatory space."),
    ...
    shape_text(bar, [Para(md_runs("**Complexity:** dominated by PCA and repeated K-Means; TreeSHAP scales with tree count and depth — **not exponentially in features**.", size=17, color=WHITE), align="ctr", lnspc=22)],
    ...
    rows = [["k", "Silhouette ↑", "Davies–Bouldin ↓", "Reading"],
            ["2", "0.214", "1.775", "best geometry, coarse semantics"],
            ["3 ★", "0.144", "2.097", "richer oenological partition — selected"]]
    ...
    textbox(s, L, ny, lw, emu(0.4), [Para([Run("Bar lengths are illustrative of rank order; exact values in Chapter 5, Fig. 5.6.", font="body", size=14, color=MUTED)], lnspc=18)])
    dx = L + lw + emu(0.6)
    dw = R - dx
    card(s, dx, top + emu(0.25), dw, CB - top - emu(0.45), label="Reading the ranking",
         paras=bullets(["Dominant drivers relate to **structure, preservation and sensory balance**.",
                        "This is **NOT an arbitrary classifier artefact** — it recovers a chemically interpretable hierarchy.",
                        "Density and pH separate stylistic regimes; sulfur dioxide encodes preservation; alcohol modulates body.",
                        "Each driver is a **modifiable, domain-level factor** — exactly Definition 1.1."],
                       size=20, color=WHITE, c0=YELLOW, c1=YELLOW, gap0=10), pad=(0.55, 0.7, 0.5, 0.3))
    ...
    light_card(s, L, cy + emu(0.25), W, CB - cy - emu(0.45), label="What the signatures show",
               paras=bullets(["Three clusters show **distinct explanatory signatures** — the explanation is coherent, not a single summary number.",
                              "The **same small set of variables recurs** across clusters, with **different relative weights** within each.",
                              "Local force plots decompose any single wine into its per-feature contributions to cluster membership."], size=21, gap0=8), pad=(0.55, 0.7, 0.5, 0.3))
    ...
    shape_text(bar, [Para(md_runs("Shapley is grounded in **four axioms**; LIME has no equivalent guarantee — with a faithful surrogate, cooperative attribution is **more stable and comparable across clusters**.", size=21, color=WHITE), align="ctr", lnspc=27)],
               anchor="ctr", insets=(emu(0.5), emu(0.1), emu(0.5), emu(0.1)))
    ...
    card(s, L, top + emu(0.25), cw, ch, label="RQ1 answered · objectives met",
         paras=bullets(["**RQ1 — yes:** faithful, coherent cluster-level explanation from Shapley values.",
                        ...
                        "**O3 met** · Shapley grounded in four axioms; LIME has no equivalent guarantee.",
                        "Recovers an oenologically interpretable ranking — density, pH, acidity, sulfur dioxide, alcohol."],
    ...
    light_card(s, L + cw + emu(0.4), top + emu(0.25), cw, ch, label="Limitations",
               paras=bullets(["Fidelity depends on the **LightGBM surrogate** — not a direct mechanism of the K-Means geometry.",
                              "Confined to **tabular data**; no structured, graph or image input.",
                              "**Single-level structure:** cannot explain how importance reconfigures between a partition and its sub-partitions.",
                              "The surrogate approximation compresses observation-level variation."], size=19, gap0=8), pad=(0.55, 0.7, 0.5, 0.3))

    # --- C1 takeaways -------------------------------------------------------
    s, top = content_slide(ctx, "Takeaways — and the Next Question", eyebrow=C1, tabs=C1TABS, active="Findings", notes=N(34))
    tk = [("Single lens", "Shapley attribution is a single, principled lens for explaining an unsupervised partition."),
          ("Original space", "Keeping attribution in the original feature space is what makes it actionable."),
          ("But…", "Real data are rarely single-level: broad regimes contain nested sub-groups.")]
    ...
    section_slide(ctx, "05", "Contribution II", "Enhanced multi-level XAI for large-scale clustering  ·  answers RQ2",
                  [("Objectives", "A genuinely multi-level workflow, a formal cross-level consistency argument (Proposition 6.1), validation on a structurally different large dataset."),
                   ...
                   ("Results", "Beijing Air Quality (383,585 records): Silhouette ≈ 0.63, three atmospheric regimes, meteorology structurally central.")],
                  notes=N(35))
    ...
    ch = CB - top - emu(0.45)
    card(s, L, top + emu(0.25), cw, ch, label="Gap",
         paras=bullets(["Once clustering is multi-level, feature importance must stay interpretable **within a cluster, across sub-clusters and across the hierarchy**.",
                        "Large-scale data make exact explanation **computationally burdensome**.",
    ...
    fit_textbox(s, x2 + emu(0.5), top + emu(0.9), cw - emu(1.0), emu(1.1), [
        P("**RQ2 ·** How can this extend to large-scale, hierarchical clustering without losing tractability or consistency?", size=20, color=GREEN)], min_scale=0.8)
    numbered_rows(s, x2 + emu(0.5), top + emu(2.05), cw - emu(1.0), [
        ("O1", "A **genuinely multi-level workflow**, not a rerun of the single-level pipeline."),
        ("O2", "A **formal cross-level consistency argument** (Proposition 6.1)."),
        ("O3", "Validation on a **structurally different large-scale dataset**."),
    ], size=19, min_h=0.9, gap=0.14)
    ...
    lw = emu(8.4)
    fit_textbox(s, L, top, lw, CB - top - emu(0.2), bullets([
        "**Recursive / nested:** coarse clustering on the full dataset, then subdivide each cluster.",
        ...
        (1, "Parent-level attribution = an **expectation over the explanatory structure of its descendants**."),
        (1, "The hierarchy is a pragmatic analytical device, not a claim of true ontological hierarchy."),
    ], size=21, gap0=12), min_scale=0.7)
    # tree diagram
    dx = L + lw + emu(0.6)
    dw = R - dx
    root = chip(s, dx + dw * 0.25, top, dw * 0.5, emu(0.95), "Level 1 · coarse clustering (k = 3)", fill=INK, size=18, radius=emu(0.2))
    regs = [("Regime A", "temp · dew point · ozone", GREEN), ("Regime B", "CO · SO₂ · PM · wind", "1E5F58"), ("Regime C", "clean air · meteorology", ORANGE)]
    ...
    card(s, dx, top + emu(0.25), dw, emu(3.3), label="Why it matters",
         paras=bullets(["Gives the multi-level explanation a **formal backbone**.",
                        "Parent importance is an **accounting identity** over children — not a separate story.",
    ...
    kpi(s, *cells[0], "k = 3", "strong convergence on **multi-criteria** evaluation", fill=GREEN)
    kpi(s, *cells[1], "≈ 0.63", "**Silhouette** — materially stronger separation than wine", fill="1E5F58")
    kpi(s, *cells[2], "≈ 0.55", "**Davies–Bouldin** — low between-cluster ambiguity", fill=ORANGE, vcolor=WHITE)
    cy = top + emu(0.1) + emu(2.3) + emu(0.45)
    cw = (W - emu(0.4)) / 2
    ch = CB - cy - emu(0.4)
    light_card(s, L, cy + emu(0.25), cw, ch, label="Setup",
               paras=bullets(["Full dataset — **383,585 hourly records**, 11 pollutant + meteorological variables.",
    ...
    card(s, L + cw + emu(0.4), cy + emu(0.25), cw, ch, label="Sensitivity",
         paras=bullets(["Robust to modest variation in **k**, projection dimension and surrogate depth.",
    ...
    s, top = content_slide(ctx, "Global SHAP Ranking — Meteorology Is Structurally Central", eyebrow=C2, tabs=C1TABS, active="Results", notes=N(41))
    ...
    textbox(s, L + label_w + emu(0.65), ly, emu(2.5), emu(0.45), [Para([Run("meteorological", font="body", size=15, color=MUTED)], lnspc=19)], anchor="ctr")
    ...
    dw = R - dx
    card(s, dx, top + emu(0.25), dw, CB - top - emu(0.45), label="Reading the ranking",
         paras=bullets(["It is **NOT simply pollutant concentrations** that matter — meteorological variables play a **structurally central role**.",
                        "Temperature, dew point and pressure condition **dispersion, trapping and photochemical behaviour**.",
                        "This is the kind of insight flat descriptive summaries often fail to make explicit."],
                       size=20, color=WHITE, c0=YELLOW, c1=YELLOW, gap0=10), pad=(0.55, 0.7, 0.5, 0.3))

    # --- C2 results: regimes -------------------------------------------------
    s, top = content_slide(ctx, "Three Atmospheric Regimes", eyebrow=C2, tabs=C1TABS, active="Results", notes=N(42))
    regs = [("Regime A", "Warm photochemical", "Ozone, temperature and dew point prominent — summer photochemical smog.", GREEN),
            ("Regime B", "Wintertime smog", "CO, SO₂ and particulate matter dominate; low wind speed suppresses dispersion.", "1E5F58"),
            ("Regime C", "Comparatively clean air", "Favourable meteorology, weak pollutant pushes.", ORANGE)]
    ...
    s, top = content_slide(ctx, "How Importance Reconfigures Across Levels", eyebrow=C2, tabs=C1TABS, active="Results", notes=N(43))
    cw = (W - emu(0.4)) / 2
    ch = emu(3.4)
    card(s, L, top + emu(0.25), cw, ch, label="Coarse level — regime selection",
         paras=bullets(["**Temperature and dew point dominate** — they differentiate broad atmospheric regimes.",
    ...
    light_card(s, L + cw + emu(0.4), top + emu(0.25), cw, ch, label="Within clusters — variation inside a regime",
               paras=bullets(["**CO, SO₂, PM10, wind speed, pressure or ozone** become more discriminative.",
                              "Cluster-level story = what varies once the regime is fixed."], size=20, gap0=10), pad=(0.55, 0.7, 0.5, 0.3))
    gy = top + emu(0.25) + ch + emu(0.4)
    box = rrect(s, L, gy, W, CB - gy - emu(0.15), fill=None, line=ORANGE, line_w=2.5, radius=emu(0.3))
    fit_textbox(s, L + emu(0.5), gy + emu(0.25), W - emu(1.0), CB - gy - emu(0.15) - emu(0.5), [
        P("**This change is NOT contradictory — it is exactly what a multi-level explanation should reveal.**", size=23, color=INK),
        P("A variable can be **globally important yet locally uninformative** within a sub-cluster; Proposition 6.1 guarantees the two readings are consistent up to surrogate residual.", size=20, spcbef=10),
    ...
    rows = [["", "Wine Quality (C1)", "Beijing Air Quality (C2)"],
            ...
            ["Nature", "small, dense, chemically correlated", "large, noisy, temporally & meteorologically variable"],
            ["Structure", "single-level", "multi-level (nested regimes)"],
            ["Silhouette", "0.144 (k* = 3, chosen for semantics)", "≈ 0.63 (k = 3)"],
            ["Explanatory logic", "Shapley in original feature space", "same logic — remains productive"]]
    ...
    card(s, L, by + emu(0.25), cw, CB - by - emu(0.45), label="vs the literature",
         paras=bullets(["The same explanatory logic is productive on **both datasets** → not tied to one domain-specific peculiarity.",
    ...
    light_card(s, L + cw + emu(0.4), by + emu(0.25), cw, CB - by - emu(0.45), label="vs LIME",
               paras=bullets(["Weaker **structural coherence** across levels.",
                              "Less stable local narratives for **hierarchical reasoning**."], size=19, gap0=8), pad=(0.55, 0.7, 0.5, 0.3))
    ...
    card(s, L, top + emu(0.25), cw, ch, label="RQ2 answered · objectives met",
         paras=bullets(["**RQ2 — yes, with bounds:** coherence retained under scale and hierarchy.",
                        "**O1 met** · a multi-granular explanation that does not collapse into a single flat summary.",
                        "**O2 met** · Proposition 6.1 provides a formal cross-level consistency argument.",
                        "**O3 met** · validated on a structurally different large-scale dataset (Beijing).",
                        "Differences across levels are **interpretable**, not dismissed as inconsistency."],
                       size=19, color=WHITE, c0=YELLOW, c1=YELLOW, gap0=8), pad=(0.55, 0.7, 0.5, 0.3))
    light_card(s, L + cw + emu(0.4), top + emu(0.25), cw, ch, label="Limitations",
               paras=bullets(["Clustering remains **static**, even though the Beijing data are temporal.",
                              "Surrogate-based SHAP plus representative-instance reporting **compress observation-level variation**.",
                              "Confined to **tabular data**.",
    ...
    tk = [("Coherent", "Shapley attribution stays coherent across granularity — when the hierarchy is explicit."),
    ...
    ch = CB - top - emu(0.45)
    card(s, L, top + emu(0.25), cw, ch, label="Gap",
         paras=bullets(["Graph and hypergraph recommenders treat message importance as **uniform or attention-weighted**, without a principled **marginal-contribution** account.",
                        "Diversity is often a secondary objective or a **re-ranking heuristic**.",
                        "Interpretability is **added after prediction**, not integrated into the learning objective."],
                       size=20, color=WHITE, c0=YELLOW, c1=YELLOW, gap0=10), pad=(0.55, 0.7, 0.5, 0.3))
    x2 = L + cw + emu(0.4)
    rrect(s, x2, top + emu(0.25), cw, ch, fill=TINT, radius=emu(0.34))
    ...
    fit_textbox(s, x2 + emu(0.5), top + emu(0.85), cw - emu(1.0), emu(1.55), [
        P("**RQ3 ·** Can cooperative attribution move beyond post-hoc and enter the learning dynamics of graph recommenders?", size=18, color=GREEN),
        P("**RQ4 ·** Can a recommender jointly optimise ranking accuracy, context and diversity when importance is estimated by a cooperative-game utility?", size=18, color=GREEN, spcbef=6)], min_scale=0.75)
    numbered_rows(s, x2 + emu(0.5), top + emu(2.6), cw - emu(1.0), [
        ...
        ("O3", "Improve **ranking, coverage and diversity jointly**."),
    ...
    card(s, L + cw + emu(0.4), cy + emu(0.25), cw, ch, label="Tuning & scope",
         paras=bullets(["Weights **grid-searched**; stable — **< 1.5 % variance** in NDCG@20 around the optimum.",
                        "Coalition evaluation scoped to the **interaction episode** (a few dozen players), not the full catalogue."],
                       size=20, color=WHITE, c0=YELLOW, c1=YELLOW, gap0=10), pad=(0.55, 0.7, 0.5, 0.3))
    ...
    shape_text(bar, [Para(md_runs("**Reduced filter-bubble effect and greater discovery opportunity** — while NDCG and Recall also improve, so **accuracy is not traded off for diversity**.", size=22, color=WHITE), align="ctr", lnspc=28)],
               anchor="ctr", insets=(emu(0.5), emu(0.1), emu(0.5), emu(0.1)))
    ...
    card(s, dx, top + emu(0.25), dw, CB - top - emu(0.45), label="Reading",
         paras=bullets(["**Context** is the most influential component (−8.2 % / −11.0 %).",
                        "**Hypergraph** structure second (−6.8 % / −8.9 %).",
                        "Removing **Shapley** weighting costs 4.6 % / 6.1 % — larger on the sparser dataset.",
                        "Ablation is component-wise; it does not test factorial interactions."],
    ...
    light_card(s, L, cy + emu(0.25), W, CB - cy - emu(0.45), label="Protocol",
               paras=bullets(["Paired t-tests on **per-user NDCG@20** (n = 6,040 users, MovieLens-1M).",
                              "DyHuCoG outperforms **every baseline** with extremely small p-values after Holm–Bonferroni correction.",
                              "**Wilcoxon signed-rank test** also significant (p < 0.001) — the result does not depend on normality.",
                              "**Effect sizes are large** — improvements are substantively meaningful, not merely statistically visible."], size=21, gap0=9), pad=(0.55, 0.7, 0.5, 0.3))
    ...
    card(s, L, top + emu(0.25), cw, ch, label="Cold-start (≤ 5 interactions)",
         paras=bullets(["NDCG@20 ≈ **0.061** (user) and **0.057** (item).",
                        "Improving over HPCF by **≈ 10 %** — Shapley weighting helps exactly where the signal is weakest."], size=20, color=WHITE, c0=YELLOW, c1=YELLOW, gap0=10), pad=(0.55, 0.7, 0.5, 0.3))
    x2 = L + cw + emu(0.4)
    rrect(s, x2, top + emu(0.25), cw, ch, fill=TINT, radius=emu(0.34))
    ...
    light_card(s, L, gy + emu(0.25), W, CB - gy - emu(0.45), label="Interpretability & popularity bias",
               paras=bullets(["A **SHAP waterfall** decomposes a recommendation into ranking, diversity, context and preference contributions.",
                              "Shapley measures **marginal utility, not raw frequency** — weak but informative interactions retain influence, which counters popularity bias."], size=21, gap0=9), pad=(0.55, 0.7, 0.5, 0.3))
    ...
    card(s, L, top + emu(0.25), cw, ch, label="RQ3 & RQ4 answered · objectives met",
         paras=bullets(["**RQ3 — yes:** attribution becomes an **in-training signal**, not a post-hoc diagnostic.",
                        ...
                        "The accuracy–diversity–context trade-off is **not structurally fixed**; largest gains on the sparsest data."],
                       size=18, color=WHITE, c0=YELLOW, c1=YELLOW, gap0=7), pad=(0.55, 0.7, 0.5, 0.3))
    light_card(s, L + cw + emu(0.4), top + emu(0.25), cw, ch, label="Limitations",
               paras=bullets(["Measurable computational overhead — roughly **1.78× training time** over HPCF.",
                              ...
                              "Baselines finalised through early 2026 — superiority is claimed **only against the tested baselines**."], size=18, gap0=7), pad=(0.55, 0.7, 0.5, 0.3))

    # --- C3 takeaways -------------------------------------------------------
    s, top = content_slide(ctx, "Takeaways", eyebrow=C3, tabs=C1TABS, active="Findings", notes=N(67))
    tk = [("First-class", "Attribution is a first-class part of the learning objective, not a post-hoc diagnostic."),
          ("Read-out", "The explanation is a direct read-out of the objective the model already optimises."),
          ("Faithful", "This makes the explanation structurally faithful rather than an external approximation."),
          ("Axiomatic", "Built on a principled, axiomatic basis — aligned with trustworthy-AI expectations.")]
    ...
    section_slide(ctx, "07", "Conclusion", "Synthesis, publications, limitations and perspectives",
                  [("Synthesis", "Three contributions, one cooperative-game thread: faithful explanation → hierarchical coherence → in-training attribution."),
                   ("Limitations", "Approximation everywhere; surrogate fidelity; tabular and offline settings; no human-subject actionability study yet."),
                   ("Perspectives", "Scalable attribution, online/streaming recommendation, human-centred and trustworthy-AI evaluation.")],
                  notes=N(68))
    ...
    rows = [["", "Main idea", "Achievement", "Key finding"],
            ...
            ["C2", "Multi-level, large-scale clustering XAI", "Cross-level SHAP aggregation + Proposition 6.1", "Coherent under hierarchy; interprets differences across levels"],
    ...
    lims = [("Computational", "Exact Shapley is intractable; every contribution relies on approximation, surrogates or restricted reporting.", GREEN),
            ("Methodological", "Clustering depends on surrogate fidelity; recommendation depends on stable approximate contributions and adequate context.", "1E5F58"),
            ("Empirical", "Tabular clustering + benchmark recommendation; no multimodal, sequential or online deployment; no dedicated human-subject actionability study.", ORANGE),
            ("Claim scope", "A coherent and productive perspective — not one fully unified framework eliminating all tension.", INK)]
    ...
    per = [("01", "Scalable cooperative attribution", "Lower-variance Shapley estimators, learned proposal distributions, adaptive refresh policies."),
           ("02", "Online / streaming recommendation", "Truly incremental settings with evolving graphs and delayed feedback — also addresses the static-graph limitation."),
           ("03", "Richer human-centred evaluation", "Do explanations measurably improve analyst judgement, user trust, intervention quality or perceived fairness?"),
           ("04", "Broader trustworthy-AI evaluation", "Exposure fairness, transparency requirements, governance-oriented auditing.")]
    cells = grid(4, 2, L, top + emu(0.1), W, CB - top - emu(0.3), gap=emu(0.3), vgap=emu(0.3))
    ...
    shape_text(box, [Para([Run("THESIS ANSWER", font="xbold", size=14, color=YELLOW, spc=1.8)], align="ctr", lnspc=18),
                     Para(md_runs("Cooperative game theory can function as a **shared methodological perspective for actionable explanation** across clustering and recommendation.", size=24, color=WHITE, font="bold", bold_font="xbold"), align="ctr", lnspc=30, spcbef=8)],
               anchor="ctr", insets=(emu(0.5), emu(0.1), emu(0.5), emu(0.1)))
    outs = [("Common language", "Shapley attribution as a common formal language for feature, interaction and context importance allocation."),
            ("Three achievements", "Faithful clustering explanation, hierarchical explanatory coherence, contribution-aware recommendation learning."),
            ("Explanation as method", "Not commentary — from post-hoc description to in-training guidance."),
